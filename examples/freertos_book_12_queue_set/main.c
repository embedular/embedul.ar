...
#error This example does not work with static allocation, there is no static equivalent of xQueueCreateSet(). See queue.c:2905
...
static StackType_t sender1Stack[ 1000 ];
static StackType_t sender2Stack[ 1000 ];
static StackType_t receiverStack[ 1000 ];

/* Task control block for xTaskCreateStatic() */
static StaticTask_t sender1ControlBlock;
static StaticTask_t sender2ControlBlock;
static StaticTask_t receiverControlBlock;
...
uint8_t ucQueue1StorageArea[ 1 * sizeof( char * ) ];
uint8_t ucQueue2StorageArea[ 1 * sizeof( char * ) ];
...
( void ) param;
    ...
    xQueue1 = xQueueCreateStatic( 1, sizeof( char * ), ucQueue1StorageArea, &xStaticQueue1  );
	xQueue2 = xQueueCreateStatic( 1, sizeof( char * ), ucQueue2StorageArea, &xStaticQueue2  );
	...
	xTaskCreateStatic( vSenderTask1, "Sender1", 1000, NULL, 1, sender1Stack, &sender1ControlBlock );
	xTaskCreateStatic( vSenderTask2, "Sender2", 1000, NULL, 1, sender2Stack, &sender2ControlBlock );

	/* Create the receiver task. */
	xTaskCreateStatic( vReceiverTask, "Receiver", 1000, NULL, 2, receiverStack, &receiverControlBlock );
