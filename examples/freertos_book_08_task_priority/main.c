...
( void ) param;
    ...
    xTaskCreateStatic( vTask1, "Task 1", 1000, NULL, 2, xTask1Stack, &xTask1ControlBlock );
                /* The task is created at priority 2 ^. */

    /* Create the second task at priority 1 - which is lower than the priority
    given to Task1.  Again the task parameter is not used so is set to NULL. */
    xTask2Handle = xTaskCreateStatic( vTask2, "Task 2", 1000, NULL, 1, xTask2Stack, &xTask2ControlBlock );
