...
uint8_t ucQueueStorageArea[ 3 * sizeof( Data_t ) ];
...
( void ) param;
    ...
    xTaskCreateStatic( vSenderTask, "Sender1", 1000, ( void * ) &( xStructsToSend[ 0 ] ), 2, sender1Stack, &sender1ControlBlock );
    xTaskCreateStatic( vSenderTask, "Sender2", 1000, ( void * ) &( xStructsToSend[ 1 ] ), 2, sender2Stack, &sender2ControlBlock );

    /* Create the task that will read from the queue.  The task is created with
    priority 1, so below the priority of the sender tasks. */
    xTaskCreateStatic( vReceiverTask, "Receiver", 1000, NULL, 1, receiverStack, &receiverControlBlock );
