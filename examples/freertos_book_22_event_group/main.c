...
static StackType_t xBitSetterStack[ 1000 ];
static StackType_t xBitReaderStack[ 1000 ];
static StackType_t xIntGenStack[ 1000 ];

/* Task control block for xTaskCreateStatic() */
static StaticTask_t xBitSetterControlBlock;
static StaticTask_t xBitReaderControlBlock;
...
event group. */
StaticEventGroup_t xStaticEventGroup;
...
( void ) param;
    ...
    for( ;; )
    {
        ...
        if( vExitRequested() )
        {
            ...
            vTaskDelete( xTaskGetHandle( "BitSetter" ) );
            vTaskDelete( xTaskGetHandle( "BitReader" ) );
