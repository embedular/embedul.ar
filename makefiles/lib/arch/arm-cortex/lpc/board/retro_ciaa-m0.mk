...
#
# project based on a retro-ciaa board, cortex-m0 core.
#
...
ifneq ($(filter video-adapter,$(TARGET_SUBSYSTEMS)),)
    # Pixel video output bit arrangement for video adapter (when enabled)
    #
    #                   Format          Arrangement
    # BIT                                               7  6  5  4  3  2  1  0
    # PIXEL DATA        (RGB332)       [7-0]            R2 R1 R0 G2 G1 G0 B1 B0
    # GPIO3             (RGB332)       [7-0]            R2 R1 R0 G2 G1 G0 B1 B0
    #
    # MASK: A low state indicates the pin is settable and readable via the
    #       masked write and read functions.
    VIDEO_ADAPTER_GPIO_MASK := 0xFFFFFF00
    VIDEO_ADAPTER_GPIO_MASK_ADDR := 0x400F608C
    VIDEO_ADAPTER_GPIO_MPIN_ADDR := 0x400F618C
endif
...
ifneq ($(filter video-adapter,$(TARGET_SUBSYSTEMS)),)
    ...
    CFLAGS += -D'VIDEO_ADAPTER_STR="DVI"'

    # Target video adapter specialization
    OBJS += $(TARGET_DRIVERS)/board_$(TARGET_BOARD)/video_dualcore_adapter/adapter_dvi.o
