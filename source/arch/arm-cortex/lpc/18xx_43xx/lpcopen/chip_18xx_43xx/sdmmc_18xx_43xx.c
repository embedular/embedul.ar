...
static uint32_t prv_get_bits(uint32_t start, uint32_t end, uint32_t *data)
{
	uint32_t v;
	uint32_t i = end >> 5;
...
uint32_t Chip_SDMMC_GetDeviceBlocks(LPC_SDMMC_T *pSDMMC)
{
    (void) pSDMMC;
...
uint32_t Chip_SDMMC_ReadBlocks(LPC_SDMMC_T *pSDMMC, void *buffer, uint32_t start_block, uint32_t num_blocks)
{
	uint32_t cbRead = (num_blocks) * MMC_SECTOR_SIZE;
	uint32_t status = 0;
	uint32_t index;

	/* if card is not acquired return immediately */
	if (!num_blocks || (start_block + num_blocks) > g_card_info->card_info.blocknr) {
		return 0;
	}
...
uint32_t Chip_SDMMC_WriteBlocks(LPC_SDMMC_T *pSDMMC, const void *buffer, uint32_t start_block, uint32_t num_blocks)
{
	uint32_t cbWrote = num_blocks *  MMC_SECTOR_SIZE;
	uint32_t status;
	uint32_t index;
