...
void (* const g_pfnVectors[])(void) = {
    ...
    0,                              // 5 Reserved
    0,                              // 6 Reserved
    __valid_user_code_checksum,     // 7 Reserved
    0,                              // 8 Reserved
    0,                              // 9 Reserved
    0,                              // 10 Reserved
...
ResetISR(void) {
  ...
  *pCREG_M0APPMAP = (unsigned int)g_pfnVectors;
  ...
  //
  // No need to copy data sections from flash to SRAM. The host will copy
  // the whole binary image to SRAM and then start execution from there.
  //
#if 0
