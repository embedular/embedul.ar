...
{
    VIDEO_TILEMAP_DrawLite (&TILEMAP_nyancat, X, Y,
...
static void rainbow (const int32_t X, const int32_t Y, uint32_t frame)
{
    ...
    {
        VIDEO_RECT_Draw (x0-lr, y0+2, 8, 3, 0xe0);
        VIDEO_RECT_Draw (x0-lr, y0+5, 8, 3, 0xf4);
        VIDEO_RECT_Draw (x0-lr, y0+8, 8, 3, 0xfc);
        VIDEO_RECT_Draw (x0-lr, y0+11, 8, 3, 0x5c);
        VIDEO_RECT_Draw (x0-lr, y0+14, 8, 3, 0x57);
        VIDEO_RECT_Draw (x0-lr, y0+17, 8, 3, 0x6b);

        VIDEO_RECT_Draw (x1-(lr+8), y1+1, 8, 3, 0xe0);
        VIDEO_RECT_Draw (x1-(lr+8), y1+4, 8, 3, 0xf4);
        VIDEO_RECT_Draw (x1-(lr+8), y1+7, 8, 3, 0xfc);
        VIDEO_RECT_Draw (x1-(lr+8), y1+10, 8, 3, 0x5c);
        VIDEO_RECT_Draw (x1-(lr+8), y1+13, 8, 3, 0x57);
        VIDEO_RECT_Draw (x1-(lr+8), y1+16, 8, 3, 0x6b);
    }
}
...
static void star (const int32_t X, const int32_t Y, uint32_t img)
{
    ...
    VIDEO_TILEMAP_DrawLite (&TILEMAP_nyancat, X, Y, TILEMAP_nyancat_star_INDEX, img);
}
...
static void draw (const int32_t X, const int32_t Y, const uint32_t Frame)
{
    ...
    const uint32_t FX  = VIDEO_FrameNumber() << 2;
                   ...
                   const TIMER_Ticks Elapsed)
{
    ...
    switch (Action)
    {
        ...
        case SEQUENCE_Action_Run:
            ...
            if (Elapsed > 400)
            {
                VIDEO_FONT_DrawStringLite (6*8, 6*8, 0xFF, "RETRO-CIAA FRAMEWORK");
            }

            if (Elapsed > 600)
            {
                VIDEO_FONT_DrawString (VIDEO_Font(), 128, 7*8, 0x0A, 
                                 VIDEO_FONT_DP_AlingHCenter, 0, 96,
                                 CC_VcsFwkVersionStr);
            }

            if (Elapsed > 2200)
            {
                VIDEO_FONT_DrawStringLite (12*8, 11*8, 0xFF, "Presents");
            }
    ...
    switch (Action)
    {
        ...
        case SEQUENCE_Action_Run:
            ...
            if (Elapsed - data->lastElapsed >= 60)
            {
                ...
                VIDEO_GradientShift (1);
            }

            // Nyan cat centered on screen
            draw (128-(32>>1), 72-(21>>1), data->frame);

            VIDEO_FONT_DrawString (VIDEO_Font(), 128, 8, VIDEO_RGB332_SelectGradientAuto,
                                 VIDEO_FONT_DP_Shadow | VIDEO_FONT_DP_AlingHCenter, 0, 96,
                                 CC_AppNameStr);

            VIDEO_FONT_DrawString (VIDEO_Font(), 128, 16, 0x0A,
                                 VIDEO_FONT_DP_Shadow | VIDEO_FONT_DP_AlingHCenter, 0, 96,
                                 BOARD_Description());

            VIDEO_FONT_DrawString (VIDEO_Font(), 128, 24, 0x0A,
                                 VIDEO_FONT_DP_Shadow | VIDEO_FONT_DP_AlingHCenter, 0, 96,
                                 CC_VcsAppVersionStr);

            if (VIDEO_FrameNumber() & 0x20)
            {
                VIDEO_FONT_DrawString (VIDEO_Font(), 11*8, 15*8, 0xFF,
                                 VIDEO_FONT_DP_Shadow, 0, 96,
                                 "PRESS START");
            }

            // Nyan Cat scrolling copyright info
            VIDEO_FONT_DrawString (VIDEO_Font(),
                             -((Elapsed >> 5) % (data->msgGlyphs * 8)),
                             136, 0x0A, VIDEO_FONT_DP_None, 0,
...
void SPLASH_THEME_nyan_cat (void)
{
    struct NYANCAT_Data data =
    {
        ...
        .msgGlyphs  = VIDEO_FONT_CountGlyphsMax (data.msg, 128)
    };
