...
#define ESP32AT_DEFAULT_HOSTNAME                "embedular"
...
static const struct STREAM_IFACE SOCKET_ESP32AT_TCP_SERVER_IFACE =
{
    .Description    = "esp32 tcp/udp server",
                                     ...
                                     struct STREAM *const Esp32AT)
{
    ...
    STREAM_Init ((struct STREAM *)E, &SOCKET_ESP32AT_TCP_SERVER_IFACE);
}
...
{
    STREAM_Command (Esp32AT, DEVICE_COMMAND_STREAM_SET_UART_BAUD, 
                        &VARIANT_SpawnUint(ESP32AT_DEFAULT_CMD_INITIAL_SPEED));
    STREAM_Command (Esp32AT, DEVICE_COMMAND_STREAM_SET_UART_HWFLOW,
                              ...
                              const uint32_t FullSpeed)
{
    ...
    STREAM_Command (Esp32AT, DEVICE_COMMAND_STREAM_SET_UART_HWFLOW,
                        &VARIANT_SpawnBoolean(true));
}


static bool doConnect (struct STREAM *const S, struct STREAM *const Esp32AT,
                       const enum ConnectStepFlags Steps)
{
    ...
    struct STREAM_ESP32AT_TCP_SERVER *const E =
                                (struct STREAM_ESP32AT_TCP_SERVER *) S;

    rnInit (&rn, entries, 8, buf, sizeof(buf));

    if (Steps & ConnectStepFlags_PowerOn)
    {
        LOG_ContextBegin (S, LANG_MODULE_RESTART);
        {
            MIO_SET_OUTPUT_BIT_NOW (CONTROL, WirelessEnable, 0);
            TICKS_Delay (ESP32AT_POWERCYCLE_OFF_DELAY);

            MIO_SET_OUTPUT_BIT_NOW (CONTROL, WirelessEnable, 1);
            TICKS_Delay (ESP32AT_POWERCYCLE_ON_DELAY);

            LOG (S, LANG_DEFAULT_SPEED_NO_FLOW_CTRL);

            // MCU uart to module initial speed
            esp32atCmdInitialSpeed (Esp32AT);

            // check for module response
            if (!waitHelloResponse (&rn, S, Esp32AT, ESP32AT_HELLO_RETRY))
            {
                LOG_ContextEnd ();
                return false;
            }
        ...
        LOG_ContextBegin (S, LANG_MODULE_SETUP);
        {
            // Disable command store
            if (!sendAtCommand (&rn, &AT_SYSSTORE, S, Esp32AT))
            {
                LOG_ContextEnd ();
                return false;
            }
            ...
            // AT command echo off
            if (!sendAtCommand (&rn, &ATE0, S, Esp32AT))
            {
                LOG_ContextEnd ();
                return false;
            }
            ...
            // Extended error log
            if (!sendAtCommand (&rn, &AT_SYSLOG, S, Esp32AT))
            {
                LOG_ContextEnd ();
                return false;
            }
            ...
            if (!sendAtCommand (&rn, &AT_CIPMUX, S, Esp32AT))
            {
                LOG_ContextEnd ();
                return false;
            }
        ...
        // Version info
        if (!sendAtCommand (&rn, &AT_GMR, S, Esp32AT))
        {
            return false;
        }
    ...
    {
        LOG_ContextBegin (S, LANG_CONFIGURING_WIFI);
        {
            // WI-FI station mode.
            if (!sendAtCommand (&rn, &AT_CWMODE, S, Esp32AT))
            {
                LOG_ContextEnd ();
                return false;
            }
            ...
            if (!sendAtCommand (&rn, &AT_CWHOSTNAME, S, Esp32AT,
                                E->hostname))
            {
                LOG_ContextEnd ();
                return false;
            ...
            const size_t PassLength =
                strnlen (E->password, SOCKET_ESP32AT_TCP_SERVER_PASSWORD_MAX);
            ...
            if (!sendAtCommand (&rn, &AT_CWJAP, S, Esp32AT, 
                                E->ssid, E->password))
            {
                LOG_ContextEnd ();
                return false;
            ...
            if (!sendAtCommand (&rn, &AT_CWRECONNCFG, S, Esp32AT))
            {
                LOG_ContextEnd ();
                return false;
            }
    ...
    {
        LOG_ContextBegin (S, LANG_TCP_SERVER_START);
        {
            LOG_Items (1, LANG_TCP_PORT, E->tcpPort);

            if (!sendAtCommand (&rn, &AT_CIPSERVER, S, Esp32AT,
                                E->tcpPort))
            {
                LOG_ContextEnd ();
                return false;
    ...
    {
        LOG_ContextBegin (S, LANG_UDP_TRANSMISSION_START);
        {
            LOG_Items (1, LANG_UDP_PORT, E->udpPort);

            if (!sendAtCommand (&rn, &AT_CIPSTART, S, Esp32AT,
                                E->udpPort))
            {
                LOG_ContextEnd ();
                return false;
    ...
    {
        LOG_ContextBegin (S, LANG_FULL_SPEED);
        {
            if (!sendAtCommand (&rn, &AT_UART_CUR, S, Esp32AT,
                                E->cmdFullSpeed))
            {
                LOG_ContextEnd ();
                return false;
    ...
    STREAM_Timeout (Esp32AT, 0);

    // Did everything needed to initialize a fully-functional connection,
    // includingmore than a successful connection to a wifi AP.
    return true;
}
...
static void hardwareInit (struct STREAM *const S)
{
    ...
    STREAM_Command (S, DEVICE_COMMAND_STREAM_SET_HOSTNAME, 
    ...
    // Assumes ESP32 UART is currently at default speed and no flow control
...
static void connect (struct STREAM *const S)
{
    ...
    S->connected = doConnect (S, E->esp32at, ConnectStepFlags__ALL);
}
                                          ...
                                          struct VARIANT *const Value)
{
    ...
    if (DEVICE_COMMAND_CHECK(STREAM_SET_HOSTNAME))
    {
        if (!setConfString (E->hostname,
                            SOCKET_ESP32AT_TCP_SERVER_HOSTNAME_MAX, Value))
        {
            return DEVICE_CommandResult_Failed;
        }
    }
    else if (DEVICE_COMMAND_CHECK(STREAM_SET_WIFI_SSID))
    {
        if (!setConfString (E->ssid,
                       SOCKET_ESP32AT_TCP_SERVER_SSID_MAX, Value))
        {
            return DEVICE_CommandResult_Failed;
        }
    }
    else if (DEVICE_COMMAND_CHECK(STREAM_SET_PASSWORD))
    {
        if (!setConfString (E->password,
                       SOCKET_ESP32AT_TCP_SERVER_PASSWORD_MAX, Value))
        {
            return DEVICE_CommandResult_Failed;
        }
    }
    else if (DEVICE_COMMAND_CHECK(STREAM_SET_IP_TCP_PORT))
    {
        E->tcpPort = VARIANT_ToUint (Value);
    }
    else if (DEVICE_COMMAND_CHECK(STREAM_SET_IP_UDP_PORT))
    {
        E->udpPort = VARIANT_ToUint (Value);  
    }
    else if (DEVICE_COMMAND_CHECK(STREAM_SET_SPEED))
    {
...
static uint32_t dataIn (struct STREAM *const S, const uint8_t *const Data,
                        const uint32_t Octets)
{
//    struct STREAM_ESP32AT_TCP_SERVER *e =
//                                (struct STREAM_ESP32AT_TCP_SERVER *) s;
    (void) S;
    (void) Data;
    (void) Octets;
...
static void atmReset (struct STREAM_ESP32AT_TCP_SERVER *const E)
{
...
static void atmCheckMessage (struct STREAM_ESP32AT_TCP_SERVER *const E)
{
    ...
    switch (E->atmUsed)
    {
        ...
        case 17:
            if (!strncmp(E->atmBuf, "WIFI DISCONNECT\r\n", 17))
            {
                ...
                E->device.status    = STREAM_TransferStatus_Disconnected;
...
static uint32_t getNextDataSize (struct STREAM *const S)
{
    struct STREAM_ESP32AT_TCP_SERVER *const E =
                                (struct STREAM_ESP32AT_TCP_SERVER *) S;
    ...
    if (E->atmBuf[0] == '+')
    {
        ...
        if (E->atmUsed < 7)
        {
            while (E->atmUsed < 7)
            {
                ...
                if (data == '\r')
                {
                    LOG_Warn (S, LANG_UNEXPECTED_IPD_BODY);
                    LOG (S, LANG_RESINCYNG);
                    LOG_BinaryDump (S, "emBuf", (uint8_t *)E->atmBuf,
                                    E->atmUsed);
            ...
            {
                // Message origin
                E->addrOrigin = E->atmBuf[5] - 48;
            }
            else
            {
                LOG_Warn (S, LANG_INVALID_IPD_BODY);
                LOG (S, LANG_RESINCYNG);
                LOG_BinaryDump (S, "emBuf", (uint8_t *)E->atmBuf,
                                E->atmUsed);
        ...
        if (E->atmUsed >= 7)
        {
            while (E->atmUsed < sizeof(E->atmBuf))
            {
                ...
                if (data < '0' || data > '9')
                {
                    ...
                    {
                        LOG_Warn (S, LANG_INVALID_IPD_SIZE);
                        LOG (S, LANG_RESINCYNG);
                        LOG_BinaryDump (S, "emBuf", (uint8_t *)E->atmBuf,
                                        E->atmUsed);
                    ...
                    // Any other character
                    BOARD_AssertUnexpectedValue (S, (uint8_t)data);
                }
            }
            ...
            if (E->atmUsed >= sizeof(E->atmBuf))
            {
                ...
                LOG_Warn (S, LANG_AT_MSG_BIGGER_THAN_EXPECT);
                LOG_Items (2,
    ...
    else
    {
        ...
        if (E->atmUsed >= sizeof(E->atmBuf))
        {
            ...
            LOG_Warn (S, LANG_UNEXPECTEDLY_LONG_MSG);
            LOG_BinaryDump (S, "emBuf", (uint8_t *)E->atmBuf, E->atmUsed);
                         ...
                         const uint32_t Octets)
{
    ...
    if (!E->pendingData)
    {
        if ((E->pendingData = getNextDataSize(S)) == 0)
        {
            return 0;
        }
    }

    const uint32_t ReadMax = (E->pendingData < Octets)? E->pendingData : Octets;

    STREAM_OUT_ToBuffer (E->esp32at, Buffer, ReadMax);

    const uint32_t ReadCount = STREAM_Count (E->esp32at);

    if (ReadCount > ReadMax)
    {
        LOG_Warn (S, LANG_TOTAL_READ_HIGH_THAN_RECV);
        LOG_Items (2,   LANG_READ,          ReadMax,
                        LANG_RECEIVED,      ReadCount);
        BOARD_AssertState (false);
    }

    E->pendingData -= ReadCount;
